...
　セキュリティの配慮で、URLとアカウントは個別に配布させていただきます。
...
2. クローンの判定  
   + 画面の上に、JuderId、お名前、完成した判定の数を提示している。ご確認を  
   + 画面の中央には、二つのコード片を展示している。クローンであるかを判定してください。
   + 確認後、画面のしたにあるFalse、Trueのチェックボクスをクリックして、提出のブタンを押してください
   + 判定しかねるケースであれば、'neutral'を選択して提出していい（コメントあればよろしく）。　ページをリロードして直接に次のペアに進んでもよいです。

3. ケースの切り替え
   + 結果提出後、自動的に次のペアを展示する
   + ページのリロードによりも、新しくペアを展示すること 
...
今回クラス、ファイルレベルのケースもあり、このWeb Appの画面に見にくいと思う。VsCodeのようなEditorにコピペするのはお勧め  
